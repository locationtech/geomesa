...
import org.apache.accumulo.core.client.mapreduce._
...
class AccumuloTap(readOrWrite: AccessMode, scheme: AccumuloScheme) extends AccTap(scheme) with Logging {
  ...
  lazy val tableOps = new ZooKeeperInstance(options.instance, options.zooKeepers)
                        .getConnector(options.user, options.password)
                        .tableOperations()
...
class AccumuloCollector(flowProcess: FlowProcess[JobConf], tap: AccumuloTap)
  ...
  override def prepare(): Unit = {
    ...
    val outputFormat = conf.getOutputFormat.asInstanceOf[DeprecatedAccumuloOutputFormat]
...
class AccumuloScheme(val options: AccumuloSourceOptions)
  ...
  // copied from accumulo input format
  private final val INPUT_INFO_HAS_BEEN_SET = classOf[AccumuloInputFormat].getSimpleName + ".configured"
  private final val OUTPUT_INFO_HAS_BEEN_SET = classOf[AccumuloOutputFormat].getSimpleName + ".configured"
  ...
  override def sourceConfInit(fp: FlowProcess[JobConf], tap: AccTap, conf: JobConf) {
    ...
    if (!conf.getBoolean(INPUT_INFO_HAS_BEEN_SET, false)) {
      ...
      options.input.autoAdjustRanges.foreach(a => if (!a) InputFormatBase.disableAutoAdjustRanges(conf))
      options.input.localIterators.foreach(InputFormatBase.setLocalIterators(conf, _))
      options.input.offlineTableScan.foreach(InputFormatBase.setScanOffline(conf, _))
      options.input.scanIsolation.foreach(InputFormatBase.setIsolated(conf, _))
      options.input.logLevel.foreach(InputFormatBase.setLogLevel(conf, _))
    }

    conf.setInputFormat(classOf[DeprecatedAccumuloInputFormat])
  }
  ...
  override def sinkConfInit(fp: FlowProcess[JobConf], tap: AccTap, conf: JobConf) {
    ...
    if (!conf.getBoolean(OUTPUT_INFO_HAS_BEEN_SET, false)) {
      AccumuloOutputFormat.setOutputInfo(conf,
                                         options.user,
                                         options.password.getBytes(),
                                         options.output.createTable,
                                         options.output.table)
      AccumuloOutputFormat.setZooKeeperInstance(conf, options.instance, options.zooKeepers)
      options.output.threads.foreach(AccumuloOutputFormat.setMaxWriteThreads(conf, _))
      options.output.logLevel.foreach(l => AccumuloOutputFormat.setLogLevel(conf, l))
    }
    conf.setOutputFormat(classOf[DeprecatedAccumuloOutputFormat])
