...
case class AccumuloScheme(options: AccumuloSourceOptions)
  ...
  override def sourceConfInit(fp: FlowProcess[JobConf], tap: AccTap, conf: JobConf): Unit = {
    ...
    if (!AccumuloAdapter.isConnectorInfoSet(conf)) {
      AccumuloAdapter.setZooKeeperInstance(conf, input.instance, input.zooKeepers)
      AccumuloAdapter.setConnectorInfo(conf, input.user, input.password)
      AccumuloAdapter.setInputTableName(conf, input.table)
      AccumuloAdapter.setScanAuthorizations(conf, input.authorizations)
      ...
      input.logLevel.foreach(AccumuloAdapter.setLogLevel(conf, _))
    }
