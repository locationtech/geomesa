...
import org.locationtech.geomesa.core.index.{ExplainNull, IndexSchema}
...
object GeoMesaInputFormat {
  ...
  def configure(job: Job,
    ...
    val scanConfig = ds.explainQuery(featureTypeName, new Query(featureTypeName, ecql), ExplainNull)

    // use the explain results to set the accumulo input format options
    InputFormatBase.setInputTableName(job, scanConfig.table)
    if (!scanConfig.ranges.isEmpty) {
      InputFormatBase.setRanges(job, scanConfig.ranges)
    }
    if (!scanConfig.colFamilies.isEmpty) {
      InputFormatBase.fetchColumns(job,
        scanConfig.colFamilies.map(cf => new org.apache.accumulo.core.util.Pair[Text, Text](cf, null)))
    }
    scanConfig.iterators.foreach(InputFormatBase.addIterator(job, _))
