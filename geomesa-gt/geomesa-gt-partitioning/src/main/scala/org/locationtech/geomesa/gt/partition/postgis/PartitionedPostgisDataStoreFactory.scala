...
import org.locationtech.geomesa.gt.partition.postgis.PartitionedPostgisDataStoreParams._
...
/**
 * Data store factory for the partitioned PostGIS store.
 *
 * Delegates to a standard JDBC-based factory to create the underlying `JDBCDataStore`, then wraps it
 * in a `PartitionedPostgisDataStore` to manage the hidden per-row visibility column. We can't just
 * subclass the JDBC factory and wrap in `createDataStoreInternal`, as `JDBCDataStoreFactory.createDataStore`
 * is final and returns `JDBCDataStore` (which is itself final)
 */
class PartitionedPostgisDataStoreFactory extends DataStoreFactorySpi with LazyLogging {

  private val delegate = new PartitionedPostgisDataStoreFactory.JdbcFactory()

  override def getDisplayName: String = delegate.getDisplayName

  override def getDescription: String = delegate.getDescription

  override def getParametersInfo: Array[Param] = delegate.getParametersInfo

  override def canProcess(params: java.util.Map[String, _]): Boolean = delegate.canProcess(params)

  override def isAvailable: Boolean = delegate.isAvailable

  override def getImplementationHints: java.util.Map[RenderingHints.Key, _] = delegate.getImplementationHints

  override def createDataStore(params: java.util.Map[String, _]): DataStore =
    new PartitionedPostgisDataStore(delegate.createDataStore(params))

  override def createNewDataStore(params: java.util.Map[String, _]): DataStore =
    throw new UnsupportedOperationException()

  // exposed for testing - delegates to the underlying jdbc factory
  private[postgis] def createConnectionOptions(params: java.util.Map[String, _]): Map[String, String] =
    delegate.createConnectionOptions(params)
}
...
object PartitionedPostgisDataStoreFactory {
  ...
  /**
   * The underlying JDBC-based factory that performs all the standard PostGIS/partitioning setup. Not
   * registered as an SPI service - it is only ever invoked through the wrapping factory above.
   */
  private class JdbcFactory extends PostgisNGDataStoreFactory with LazyLogging {
    ...
    override protected def setupParameters(parameters: java.util.Map[String, AnyRef]): Unit = {
      ...
      PartitionedPostgisDataStoreFactory.ExtraParams.foreach(p => parameters.put(p.key, p))
    }

    override protected def createDataSource(params: java.util.Map[String, _], dialect: SQLDialect): DataSource = {
      val auths = AuthUtils.getProvider(params, AuthsParam.lookupOpt(params).getOrElse("").split(",").filterNot(_.isEmpty).toSeq)
      ...
      new SessionDataSource(metrics.getOrElse(dataSource), auths)
    }
   ...
   */
  private class Dbcp2ManagedDataSource(wrapped: org.apache.commons.dbcp2.BasicDataSource, registry: Closeable)
