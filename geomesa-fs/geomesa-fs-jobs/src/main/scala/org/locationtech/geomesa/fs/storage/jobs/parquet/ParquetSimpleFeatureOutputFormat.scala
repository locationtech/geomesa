...
class ParquetSimpleFeatureOutputFormat extends ParquetOutputFormat[SimpleFeature] {
  ...
  override def getOutputCommitter(context: TaskAttemptContext): OutputCommitter = {
    if (committer == null) {
      ...
      committer = new ParquetMultiFileOutputCommitter(output, context)
    }
    committer
  }
}
...
object ParquetSimpleFeatureOutputFormat {
  ...
  class ParquetMultiFileOutputCommitter(outputPath: Path, context: TaskAttemptContext)
  ...
  private def listFiles(path: Path, conf: Configuration): Seq[Path] = {
