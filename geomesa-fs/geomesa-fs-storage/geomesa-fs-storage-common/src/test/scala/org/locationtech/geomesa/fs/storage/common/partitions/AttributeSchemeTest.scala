...
class AttributeSchemeTest extends Specification {
  ...
  "AttributeScheme" should {
    ...
    "partition by string attribute" in {
      ...
      ps.getPartition(sf1) mustEqual "testvalue"

      val sf2 = ScalaSimpleFeature.create(sft, "2", "AnotherTest", 20, 2000000L, 5.5f, 12.34d)
      ps.getPartition(sf2) mustEqual "anothertest"
    }
    ...
    "partition by string attribute with width option" in {
      ...
      val sf1 = ScalaSimpleFeature.create(sft, "1", "TestValue", 10, 1000000L, 3.2f, 9.99d)
      ps.getPartition(sf1) mustEqual "test"

      val sf2 = ScalaSimpleFeature.create(sft, "2", "AnotherTest", 20, 2000000L, 5.5f, 12.34d)
      ps.getPartition(sf2) mustEqual "anot"

      val sf3 = ScalaSimpleFeature.create(sft, "3", "abc", 30, 3000000L, 7.8f, 15.67d)
      ps.getPartition(sf3) mustEqual "abc"
    }
    ...
    "handle null string values with default" in {
      ...
      val sf = ScalaSimpleFeature.create(sft, "1", null, 10, 1000000L, 3.2f, 9.99d)
      ps.getPartition(sf) mustEqual "unknown"
    }
    ...
    "partition by int attribute" in {
      ...
      ps.getPartition(sf1) mustEqual AttributeIndexKey.typeEncode(10)

      val sf2 = ScalaSimpleFeature.create(sft, "3", "test", 20, 3000000L, 7.8f, 15.67d)
      ps.getPartition(sf2) mustEqual AttributeIndexKey.typeEncode(20)
    }
    ...
    "partition by int attribute with divisor option" in {
      ...
      foreach(Range(10, 20)) { i =>
        ...
        ps.getPartition(sf) mustEqual AttributeIndexKey.typeEncode(10)
      }

      foreach(Range(20, 30)) { i =>
        val sf = ScalaSimpleFeature.create(sft, "1", "test", i, 1000000L, 3.2f, 9.99d)
        ps.getPartition(sf) mustEqual AttributeIndexKey.typeEncode(20)
      }
    }
    ...
    "handle null int values with implicit default" in {
      ...
      ps.getPartition(sf) mustEqual AttributeIndexKey.typeEncode(0)
    }
    ...
    "handle null int values with default" in {
      ...
      val sf = ScalaSimpleFeature.create(sft, "1", "test", null, 1000000L, 3.2f, 9.99d)
      ps.getPartition(sf) mustEqual AttributeIndexKey.typeEncode(1)
    }
    ...
    "partition by long attribute" in {
      ...
      val sf1 = ScalaSimpleFeature.create(sft, "1", "test", 10, 1000000L, 3.2f, 9.99d)
      ps.getPartition(sf1) mustEqual AttributeIndexKey.typeEncode(1000000L)

      val sf2 = ScalaSimpleFeature.create(sft, "3", "test", 10, 2000000L, 7.8f, 15.67d)
      ps.getPartition(sf2) mustEqual AttributeIndexKey.typeEncode(2000000L)
    }
    ...
    "partition by long attribute with divisor option" in {
      ...
      foreach(Seq(1050000L, 1080000L)) { i =>
        ...
        ps.getPartition(sf) mustEqual AttributeIndexKey.typeEncode(1000000L)
      }

      // 1150000 should bucket to 1100000
      val sf2 = ScalaSimpleFeature.create(sft, "3", "test", 10, 1150000L, 7.8f, 15.67d)
      ps.getPartition(sf2) mustEqual AttributeIndexKey.typeEncode(1100000L)
    }
    ...
    "handle null long values with implicit default" in {
      ...
      ps.getPartition(sf) mustEqual AttributeIndexKey.typeEncode(0L)
    }
    ...
    "handle null long values with default" in {
      ...
      val sf = ScalaSimpleFeature.create(sft, "1", "test", 10, null, 3.2f, 9.99d)
      ps.getPartition(sf) mustEqual AttributeIndexKey.typeEncode(1L)
    }
    ...
    "partition by float attribute" in {
      ...
      val sf1 = ScalaSimpleFeature.create(sft, "1", "test", 10, 1000000L, 3.2f, 9.99d)
      ps.getPartition(sf1) mustEqual AttributeIndexKey.typeEncode(3.2f)

      val sf2 = ScalaSimpleFeature.create(sft, "3", "test", 10, 1000000L, 5.5f, 15.67d)
      ps.getPartition(sf2) mustEqual AttributeIndexKey.typeEncode(5.5f)
    }
    ...
    "partition by float attribute with scale option" in {
      ...
      foreach(Seq(3.25f, 3.28f)) { i =>
        ...
        ps.getPartition(sf) mustEqual AttributeIndexKey.typeEncode(3.2f)
      }

      // 3.35 should bucket to 3.3
      val sf2 = ScalaSimpleFeature.create(sft, "3", "test", 10, 1000000L, 3.35f, 15.67d)
      ps.getPartition(sf2) mustEqual AttributeIndexKey.typeEncode(3.3f)
    }
    ...
    "handle null float values with implicit default" in {
      ...
      ps.getPartition(sf) mustEqual AttributeIndexKey.typeEncode(0f)
    }
    ...
    "handle null float values with default" in {
      ...
      val sf = ScalaSimpleFeature.create(sft, "1", "test", 10, 1000000L, null, 9.99d)
      ps.getPartition(sf) mustEqual AttributeIndexKey.typeEncode(1f)
    }
    ...
    "partition by double attribute" in {
      ...
      val sf1 = ScalaSimpleFeature.create(sft, "1", "test", 10, 1000000L, 3.2f, 9.99d)
      ps.getPartition(sf1) mustEqual AttributeIndexKey.typeEncode(9.99d)

      val sf2 = ScalaSimpleFeature.create(sft, "3", "test", 10, 1000000L, 3.2f, 12.34d)
      ps.getPartition(sf2) mustEqual AttributeIndexKey.typeEncode(12.34d)
    }
    ...
    "partition by double attribute with scale option" in {
      ...
      foreach(Seq(9.991d, 9.995d)) { i =>
        ...
        ps.getPartition(sf) mustEqual AttributeIndexKey.typeEncode(9.99d)
      }

      // 10.001 should bucket to 10.00
      val sf2 = ScalaSimpleFeature.create(sft, "3", "test", 10, 1000000L, 3.2f, 10.001d)
      ps.getPartition(sf2) mustEqual AttributeIndexKey.typeEncode(10d)
    }
    ...
    "handle null double values with implicit default" in {
      ...
      ps.getPartition(sf) mustEqual AttributeIndexKey.typeEncode(0d)
    }
    ...
    "handle null double values with default" in {
      ...
      val sf = ScalaSimpleFeature.create(sft, "1", "test", 10, 1000000L, 3.2f, null)
      ps.getPartition(sf) mustEqual AttributeIndexKey.typeEncode(1d)
    }
    ...
    "filter string values to allowed partitions" in {
      ...
      ps.getPartition(sf2) mustEqual "another"
    ...
    "filter int values to allowed partitions" in {
      ...
      ps.getPartition(sf3) mustEqual AttributeIndexKey.typeEncode(10)
    }
    ...
    "calculate intersecting partitions for string equality filter" in {
      ...
      partitions.get.head.bounds must haveLength(1)
      partitions.get.head.bounds.head must beAnInstanceOf[SinglePartition]
      partitions.get.head.bounds.head.asInstanceOf[SinglePartition].value mustEqual "testvalue"
    }
    ...
    "calculate intersecting partitions for string IN filter" in {
      ...
      partitions must beSome
      partitions.get must haveLength(1)
      partitions.get.head.bounds must haveLength(2)
      foreach(partitions.get.head.bounds)(_ must beAnInstanceOf[SinglePartition])

      val partitionStrings = partitions.get.head.bounds.map(_.asInstanceOf[SinglePartition].value)
      partitionStrings must containTheSameElementsAs(Seq("test", "another"))
    }
    ...
    "calculate intersecting partitions for string range filter" in {
      ...
      partitions.get must haveLength(1)
      partitions.get.head.bounds must haveLength(1)
      partitions.get.head.bounds.head must beAnInstanceOf[PartitionRange]
      val PartitionRange(name, lower, upper) = partitions.get.head.bounds.head.asInstanceOf[PartitionRange]
      name mustEqual ps.name
      lower mustEqual "a"
      upper mustEqual "z"
    }
    ...
    "calculate intersecting partitions for int equality filter" in {
      ...
      partitions.get.head.bounds must haveLength(1)
      partitions.get.head.bounds.head must beAnInstanceOf[SinglePartition]
      partitions.get.head.bounds.head.asInstanceOf[SinglePartition].value mustEqual AttributeIndexKey.typeEncode(10)
    }
    ...
    "calculate intersecting partitions for int range filter" in {
      ...
      val PartitionRange(name, lower, upper) = partitions.get.head.bounds.head.asInstanceOf[PartitionRange]
      name mustEqual ps.name
      lower mustEqual AttributeIndexKey.typeEncode(10)
      upper mustEqual AttributeIndexKey.typeEncode(20) + new String(Array[Byte](0), StandardCharsets.UTF_8)
    }
    ...
    "calculate intersecting partitions for int with divisor" in {
      ...
      partitions must beSome
      partitions.get must haveLength(1)
      partitions.get.head.bounds must not(beEmpty)
    }
    ...
    "calculate intersecting partitions for float with scale" in {
      val ps = PartitionSchemeFactory.load(sft, "attribute:attribute=weight:scale=1")
      val filter = ECQL.toFilter("weight >= 3.2 AND weight < 3.4")
    ...
    "calculate intersecting partitions for double with scale" in {
      val ps = PartitionSchemeFactory.load(sft, "attribute:attribute=precision:scale=2")
      val filter = ECQL.toFilter("precision >= 9.99 AND precision < 10.01")
    ...
    "handle allowed values with intersecting partitions" in {
      ...
      val partitionStrings = partitions.get.head.bounds.map(_.asInstanceOf[SinglePartition].value)
      partitionStrings must haveLength(2)
      partitionStrings must containAllOf(Seq("test", "another"))
    }
  }
}
