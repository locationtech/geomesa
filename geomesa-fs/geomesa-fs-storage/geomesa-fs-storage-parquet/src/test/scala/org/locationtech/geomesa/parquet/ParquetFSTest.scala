...
class ParquetFSTest extends Specification with AllExpectations {
  ...
  "ParquetFileSystemStorage" should {
    ...
    "write and read features" >> {
      ...
      WithClose(fsStorage.getReader(sft.getTypeName, partitions.take(1), new Query("test", ECQL.toFilter("name = 'first'")))) { reader =>
      ...
      WithClose(fsStorage.getReader(sft.getTypeName, partitions.slice(2, 3), new Query("test", ECQL.toFilter("name = 'third'")))) { reader =>
        val features = reader.toList
        features must haveSize(1)
      ...
      WithClose(fsStorage.getReader(sft.getTypeName, partitions.slice(2, 3), transform)) { reader =>
        val features = reader.toList
        features must haveSize(1)
