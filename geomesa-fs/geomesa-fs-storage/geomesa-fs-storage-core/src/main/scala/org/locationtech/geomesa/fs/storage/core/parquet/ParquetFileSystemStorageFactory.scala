...
import org.locationtech.geomesa.fs.storage.core.parquet.ParquetFileSystemStorage.ParquetCompressionOpt
import org.locationtech.geomesa.fs.storage.core.{FileSystemContext, FileSystemStorageFactory, StorageMetadata}
...
class ParquetFileSystemStorageFactory extends FileSystemStorageFactory {
  ...
  override def apply(context: FileSystemContext, metadata: StorageMetadata): ParquetFileSystemStorage = {
    ...
    new ParquetFileSystemStorage(context.copy(conf = context.conf ++ conf.result()), metadata)
  }
}
