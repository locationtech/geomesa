    ...
    "compact partitions" in {
      ...
      try {
        ...
        WithClose(new ParquetFileSystemStorageFactory().apply(context, catalog.create(sft, Seq("daily")))) { storage =>
          ...
          CloseableIterator(storage.getReader(Query.ALL, 1)).toList must haveSize(100)
          ...
          CloseableIterator(storage.getReader(Query.ALL, 1)).toList must haveSize(200)
          ...
          CloseableIterator(storage.getReader(Query.ALL, 1)).toList must haveSize(300)
          ...
          WithClose(storage.getWriter(ECQL.toFilter("IN ('2')"), 1)) { writer =>
            writer.hasNext must beTrue
            writer.next
            writer.remove()
          }
          storage.metadata.getFiles(partition) must haveSize(2)
          CloseableIterator(storage.getReader(Query.ALL, 1)).toList must haveSize(299)
          storage.compact(partition)
          storage.metadata.getFiles(partition) must haveSize(1)
          CloseableIterator(storage.getReader(Query.ALL, 1)).toList must haveSize(299)
