...
private class StatParser extends Parser {
  ...
  private def int: Rule1[Int] = rule { group(optional("-") ~ oneOrMore("0" - "9")) ~> (_.toInt) }

  private def char: Rule0 = rule { "a" - "z" | "A" - "Z" | "0" - "9" | "_" }

  private def string: Rule1[String] = rule { quotedString | singleQuotedString | unquotedString }

  private def unquotedString: Rule1[String] = rule { oneOrMore(char) ~> { c => c } }

  private def quotedString: Rule1[String] = rule {
    "\"" ~ zeroOrMore((noneOf("""\"""") ~? notControlChar) | escapedChar) ~> StringEscapeUtils.unescapeJava ~ "\""
  }

  private def singleQuotedString: Rule1[String] = rule {
    "'" ~ zeroOrMore((noneOf("""\'""") ~? notControlChar) | escapedChar) ~> StringEscapeUtils.unescapeJava ~ "'"
  }
