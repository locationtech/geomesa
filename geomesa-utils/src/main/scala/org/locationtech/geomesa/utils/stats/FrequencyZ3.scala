...
/**
  ...
  */
class FrequencyZ3(val geomIndex: Int,
                  val dtgIndex: Int,
                  val precision: Int,
                  val eps: Double = 0.005,
                  val confidence: Double = 0.95) extends Stat {

  override type S = FrequencyZ3
  ...
  private def toKey(geom: Geometry, dtg: Date): (Short, Long) = {
    ...
    val week = Weeks.weeksBetween(FrequencyZ3.Epoch, time).getWeeks
    val secondsInWeek = Seconds.secondsBetween(FrequencyZ3.Epoch, time.minusWeeks(week)).getSeconds
  ...
  /**
    ...
    * @return
    */
  def splitByWeek: Seq[(Short, FrequencyZ3)] = {
    sketches.toSeq.map { case (w, sketch) =>
      val freq = new FrequencyZ3(geomIndex, dtgIndex, precision, eps, confidence)
  ...
  override def +(other: FrequencyZ3): FrequencyZ3 = {
    val plus = new FrequencyZ3(geomIndex, dtgIndex, precision, eps, confidence)
    plus += this
    plus += other
    plus
  }

  override def +=(other: FrequencyZ3): Unit = {
