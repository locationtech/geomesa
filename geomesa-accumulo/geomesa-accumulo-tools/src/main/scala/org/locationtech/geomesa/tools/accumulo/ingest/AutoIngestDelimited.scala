...
object AutoIngestDelimited {
  ...
  def getCsvFormat(format: Formats): CSVFormat = format match {
 ...
 */
class DelimitedIngestConverter(ds: DataStore, typeName: String, format: Formats) extends LocalIngestConverter {
 ...
 * @param format csv or tsv
 */
class DelimitedIngestJob(typeName: String, format: Formats) extends AbstractIngestJob {
...
class DelimitedIngestRecordReader extends FileStreamRecordReader {
  ...
  override def createIterator(stream: InputStream with Seekable,
    ...
    val format = Formats.withName(context.getConfiguration.get(AutoIngestDelimited.FormatConfig))
