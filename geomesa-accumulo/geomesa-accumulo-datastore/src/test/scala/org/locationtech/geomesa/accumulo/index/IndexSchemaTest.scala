...
class IndexSchemaTest extends Specification {
  ...
  val dummyBinEncoder = BinEncoder(dummyType)
  val customBinEncoder = BinEncoder(customType)
  ...
  "single-point (-78.4953560 38.0752150)" should {
    "encode to 2 index rows" in {
      ...
      val toWrite = WritableFeature(item, dummyType, "", dummyEncoder, dummyIndexValueEncoder, dummyBinEncoder)
  ...
  "index-entry encoded and decoder" should {
    "encode and decode round-trip properly using a custom date-time field name" in {
      ...
      val toWrite = WritableFeature(entry, customType, "", customEncoder, dummyIndexValueEncoder, customBinEncoder)
