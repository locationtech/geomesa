...
class QueryStrategyDeciderTest extends Specification with TestWithDataStore {
  ...
  override val spec = "nameHighCardinality:String:index=true:cardinality=high,ageJoinIndex:Long:index=true," +
      "heightFullIndex:Float:index=full,weightNoIndex:String,dtgNoIndex:Date,dtg:Date,*geom:Point:srid=4326"
  ...
  addFeatures {
    ...
    (0 until 1000).map { i =>
      ...
      sf.setAttributes(Array(name, age, height, weight, noDtg, dtg, geom).asInstanceOf[Array[AnyRef]])
      sf
    }
  }
  ...
  "Cost-based strategy decisions" should {
    ...
    def getStrategies(filter: Filter): Seq[QueryFilter] = ds.getQueryPlan(new Query(sftName, filter)).map(_.filter)

    def getStrategy(filter: String, expected: StrategyType) = {
      val strategies = getStrategies(ECQL.toFilter(filter))
      strategies must haveLength(1)
      strategies.head.strategy mustEqual expected
    }
    ...
    "select strategy based on expected cost" >> {
      getZ2Strategy("bbox(geom,-120,45,-119,55) AND nameHighCardinality > 'name001'")
      getZ2Strategy("bbox(geom,-75,45,-70,55) AND ageJoinIndex > 30")
      getZ3Strategy("bbox(geom,-75,45,-70,55) AND dtg DURING 2016-03-01T00:00:00.000Z/2016-03-07T00:00:00.000Z AND " +
          "ageJoinIndex > 30")
      getAttributeStrategy("bbox(geom,-75,45,-70,55) AND dtg DURING 2016-03-01T00:00:00.000Z/2016-03-07T00:00:00.000Z AND " +
          "nameHighCardinality > 'name990'")
      getAttributeStrategy("bbox(geom,-75,45,-70,55) AND dtg DURING 2016-03-01T00:00:00.000Z/2016-03-07T00:00:00.000Z AND " +
          "nameHighCardinality IN ('name990', 'name991', 'name992', 'name993', 'name994')")
      getRecordStrategy("bbox(geom,-75,45,-70,55) AND dtg DURING 2016-03-01T00:00:00.000Z/2016-03-07T00:00:00.000Z AND " +
          "nameHighCardinality > 'name990' AND IN('name001', 'name002', 'name003')")
    }

    "select strategies that should result in zero rows scanned" >> {
      getZ2Strategy("bbox(geom,-75,0,-70,10)AND nameHighCardinality IN ('name990', 'name991', 'name992', 'name993', 'name994')")
      getAttributeStrategy("bbox(geom,-75,45,-70,55) AND dtg DURING 2016-03-01T00:00:00.000Z/2016-03-07T00:00:00.000Z AND " +
    ...
    "Indexed attribute filters should" >> {
      "get the attribute strategy for indexed attributes" >> {
        val predicates = Seq(
          ...
          "ageJoinIndex > 11",
          "11 > ageJoinIndex",
          "ageJoinIndex DURING 2012-01-01T11:00:00.000Z/2014-01-01T12:15:00.000Z",
          "ns:ageJoinIndex DURING 2012-01-01T11:00:00.000Z/2014-01-01T12:15:00.000Z",
          "ageJoinIndex AFTER 2013-01-01T12:30:00.000Z",
          "ageJoinIndex BEFORE 2014-01-01T12:30:00.000Z",
          "ns:ageJoinIndex BEFORE 2014-01-01T12:30:00.000Z",
