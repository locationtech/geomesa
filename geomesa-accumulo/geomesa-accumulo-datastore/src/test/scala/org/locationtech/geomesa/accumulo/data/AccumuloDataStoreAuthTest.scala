...
class AccumuloDataStoreAuthTest extends TestWithDataStore with org.specs2.matcher.SequenceMatchersCreation {
    ...
    "provide ability to configure authorizations" >> {
      ...
      "fail when auth provider system property does not match an actual class" >> {
        ...
        try {
          ...
          DataStoreFinder.getDataStore(Map(
            "connector" -> connector,
            "tableName" -> sftName,
            "auths"     -> "user,admin,test")) must throwAn[IllegalArgumentException]
        } finally {
      ...
      "fail when authorizations are explicitly provided, but the flag to force using authorizations is not set" >> {
        ...
        DataStoreFinder.getDataStore(Map(
          ...
          "forceEmptyAuths" -> java.lang.Boolean.TRUE)) must throwAn[IllegalArgumentException]
      }
