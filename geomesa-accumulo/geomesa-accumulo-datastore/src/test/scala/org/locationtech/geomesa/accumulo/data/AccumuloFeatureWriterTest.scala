...
class AccumuloFeatureWriterTest extends Specification with TestWithMultipleSfts with BeforeEach {
  ...
  override def before: Any = {
    sfts.foreach{ sft =>
      ds.manager.indices(sft).flatMap(_.getTableNames(sft, ds)).foreach { name =>
  ...
  "AccumuloFeatureWriter" should {
    ...
    "provide ability to remove features" in {
      foreach(sfts) { sft =>
        ...
        forall(ds.manager.indices(sft).flatMap(_.getTableNames(sft, ds))) { name =>
    ...
    "not write partial features" in {
      foreach(sfts) { sft =>
        ...
        forall(invalid) { feature =>
          ...
          forall(ds.manager.indices(sft).flatMap(_.getTableNames(sft, ds))) { table =>
