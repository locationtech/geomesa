...
import org.locationtech.geomesa.accumulo.data.tables.{EnabledTables, GeoMesaTable, RecordTable}
...
class AccumuloFeatureWriterTest extends Specification with TestWithDataStore with BeforeExample {
  ...
  "AccumuloFeatureWriter" should {
    ...
    "verify delete and add same key works" in {
      ...
      QueryStrategyDecider.chooseStrategies(sft, EnabledTables.AllTables, q, hints, None).head must beAnInstanceOf[AttributeIdxStrategy]
  ...
  def clearTablesHard(): Unit = {
    GeoMesaTable.getTables(sft, ds.getEnabledTables).map(ds.getTableName(sft.getTypeName, _)).foreach { name =>
