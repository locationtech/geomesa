...
class BinVisitor(sft: SimpleFeatureType, options: EncodingOptions)
  ...
  def binQuery(source: SimpleFeatureSource, query: Query): Unit = {
    ...
    options.geomField.foreach { case GeometryAttribute(g, _) => query.getHints.put(QueryHints.BIN_GEOM_KEY, g) }
    options.dtgField.foreach(query.getHints.put(QueryHints.BIN_DTG_KEY, _))
