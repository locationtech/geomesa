 ...
 */
class QueryFilterSplitter(sft: SimpleFeatureType, availableTables: List[GeoMesaTable]) extends Logging {

  val supported = GeoMesaTable.getTables(sft, availableTables)
