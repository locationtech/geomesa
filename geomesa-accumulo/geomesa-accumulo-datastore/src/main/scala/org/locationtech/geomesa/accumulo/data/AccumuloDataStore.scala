...
class AccumuloDataStore(val connector: Connector,
                        ...
                        val config: AccumuloDataStoreConfig = AccumuloDataStoreConfig())
  ...
  private val recordScanThreads = config.recordThreads.getOrElse(10)

  private val writeThreads = config.writeThreads.getOrElse(10)
  ...
  private def deleteSharedTables(sft: SimpleFeatureType) = {
    ...
    val numThreads = config.queryThreads.getOrElse(Math.min(MAX_QUERY_THREADS,
  ...
  override def getSuggestedThreads(featureName: String, table: GeoMesaTable): Int = {
    table match {
      ...
      case Z3Table             => config.queryThreads.getOrElse(8)
      case AttributeTable      => config.queryThreads.getOrElse(8)
      case AttributeTableV5    => 1
      case SpatioTemporalTable =>
        config.queryThreads.getOrElse {
...
case class AccumuloDataStoreConfig(queryTimeout: Option[Long] = None,
                                   queryThreads: Option[Int] = None,
                                   recordThreads: Option[Int] = None,
