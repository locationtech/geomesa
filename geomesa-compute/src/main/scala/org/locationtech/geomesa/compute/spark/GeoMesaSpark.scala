...
object GeoMesaSpark {
  ...
  def rdd(conf: Configuration, sc: SparkContext, ds: AccumuloDataStore, query: Query): RDD[SimpleFeature] = {
    ...
    val user      = ds.connector.whoami
    val passwd    = ds.authToken
    val auths     = ds.authorizationsProvider.getAuthorizations

    InputFormatBase.setInputInfo(conf, user, passwd, tableName, auths)
    InputFormatBase.setZooKeeperInstance(conf, ds.connector.getInstance.getInstanceName, ds.connector.getInstance().getZooKeepers)
    InputFormatBase.setRanges(conf, qp.ranges)
    qp.iterators.foreach { is => InputFormatBase.addIterator(conf, is) }
