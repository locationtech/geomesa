...
class SpatialRelationFunctionsTest extends TestWithSpark with BeforeAll {
  ...
  lazy val dfPoints: DataFrame = points.mapValues(WKTUtils.read).toSeq.toDF("name", "geom")
  lazy val dfLines: DataFrame = lines.mapValues(WKTUtils.read).toSeq.toDF("name", "geom")
  lazy val dfBoxes: DataFrame = boxes.mapValues(WKTUtils.read).toSeq.toDF("name", "geom")
