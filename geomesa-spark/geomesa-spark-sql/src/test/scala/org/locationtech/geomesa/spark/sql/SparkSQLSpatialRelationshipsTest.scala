...
class SparkSQLSpatialRelationshipsTest extends TestWithSpark with BeforeAfterAll {

  val params = Collections.singletonMap(TestSpatialProvider.StoreParam, getClass.getSimpleName)
  val ds = TestSpatialProvider.newStore(params)
  ...
  override def afterAll(): Unit = TestSpatialProvider.dispose(params)
