    ...
    val lineFeatures =
      data.features().toList
        ...
        val results = groups.flatMap { sorted =>
          ...
          val pts = sorted.map(_.point.getCoordinate)
          pts.sliding(2).zipWithIndex.map { case (ptLst, idx) =>
            val length = JTS.orthodromicDistance(ptLst.head, ptLst.last, DefaultGeographicCRS.WGS84)
            val sf = builder.buildFeature(s"$group-$idx", Array[AnyRef](gf.createLineString(ptLst.toArray)))
            (length, sf)
          }
        }
