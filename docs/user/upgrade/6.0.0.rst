...
The file system data store has been substantially rewritten to improve performance. As such, feature types created with previous
versions of GeoMesa are no longer readable. Data can be migrated by re-creating the schema and using the ``manage-metadata`` CLI
command to re-register the existing data files.
