...
(e.g. ``attr:foo:dtg`` instead of ``attr:foo:geom:dtg``). The old behavior can still be achieved by fully specifying the
secondary attributes - see :ref:`customizing_index_creation` for details.
