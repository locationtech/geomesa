...
GeoMesa no longer supports Accumulo 2.0. Users should upgrade to the latest Accumulo 2.1.x.
...
GeoMesa no longer supports Spark 3.x. Users should upgrade to Spark 4. Note that only ``_2.13`` Spark artifacts
are published, as Spark 4 does not support Scala 2.12.

Kafka Zookeeper Deprecation
---------------------------
...
FileSystem Data Store Compatibility
-----------------------------------

The file system data store has been substantially rewritten to improve performance. As part of this, any previous data
is no longer compatible, and schemas will have to be re-created and re-populated.
...
Partitioned PostGIS Data Store Class Change
-------------------------------------------

In order to support visibilities, the partitioned PostGIS data store is no longer an instance of ``JDBCDataStore``. This
prevents GeoServer from providing SQL views and SQL virtual tables against the store. It may also affect any code that checks
runtime instance types. If necessary, the underlying data store can be accessed by checking for
``org.geotools.data.store.DecoratingDataStore`` and using the ``isWrapperFor`` / ``unwrap`` methods.

Removal of ``geomesa-site.xml``
-------------------------------

The ``geomesa-site.xml`` file is no longer supported. System properties should be configured through standard environment
variables instead (e.g. ``CUSTOM_JAVA_OPTS`` for the GeoMesa CLI, ``CATALINA_OPTS`` for Tomcat, ``JAVA_TOOL_OPTIONS``, etc).
...
* accumulo ``2.1.4`` -> ``2.1.6``
* aircompressor ``0.27`` -> ``2.0.3``
* arrow ``18.3.0`` -> ``19.0.0``
* avro ``1.11.4`` -> ``1.12.1``
* aws-crt ``0.39.4`` -> ``0.46.1``
* aws-sdk ``2.38.7`` -> ``2.44.14``
* cassandra-driver ``3.11.5`` -> ``4.17.0``
...
* geotools ``33.2`` -> ``35.0``
* hadoop ``3.4.2`` -> ``3.5.0``
* jackson ``2.19.0`` -> ``2.22.3``/``3.2.3``
* jcommander ``1.82`` -> ``3.0``
* kafka ``3.9.0`` -> ``3.9.2``
* micrometer ``1.15.4`` -> ``1.17.0``
* nifi ``2.6.0`` -> ``2.9.0``
* parquet ``1.15.2`` -> ``1.18.0``
...
* spark ``3.5.5`` -> ``4.2.0``
* spring ``5.3.39`` -> ``7.0.8``
* spring-security ``5.8.16`` -> ``7.1.0``
* typesafe-config ``1.4.3`` -> ``1.4.9``
...
* ``org.locationtech.geomesa:geomesa-accumulo-spark-runtime-accumulo21_2.13:jar`` -> ``org.locationtech.geomesa:geomesa-accumulo-spark_2.13:jar:runtime``
* ``org.locationtech.geomesa:geomesa-fs-spark-runtime_2.13:jar`` -> ``org.locationtech.geomesa:geomesa-fs-spark_2.13:jar:runtime``
* ``org.locationtech.geomesa:geomesa-gt-spark-runtime_2.13:jar`` -> ``org.locationtech.geomesa:geomesa-gt-spark_2.13:jar:runtime``
* ``org.locationtech.geomesa:geomesa-hbase-spark-runtime-hbase2_2.13:jar`` -> ``org.locationtech.geomesa:geomesa-hbase-spark_2.13:jar:runtime``
...
* ``org.locationtech.geomesa.features.exporters.FileSystemExporter.ParquetFileSystemExporter`` ->
  ``org.locationtech.geomesa.features.exporters.GeoParquetExporter``
* ``org.locationtech.geomesa.features.kryo.json.JsonPathFilterFunction`` -> ``org.locationtech.geomesa.utils.json``
* ``org.locationtech.geomesa.features.kryo.json.JsonPathParser`` -> ``org.locationtech.geomesa.utils.json``
* ``org.locationtech.geomesa.features.kryo.json.JsonPathPropertyAccessor`` -> ``org.locationtech.geomesa.utils.json``
...
Most of the classes under ``org.locationtech.geomesa.fs.storage`` have been modified, moved or removed.
