...
The file system data store has been substantially rewritten to improve performance. As part of this, the Orc file format is no
longer supported, and the Parquet file format is only supported if created with GeoMesa version ``5.3.0`` or later. For supported
formats, layers must be migrated by re-creating the schema using the latest GeoMesa version, and then re-registering the existing
data files using the ``manage-metadata register`` CLI command.
