...
The XML converter processes XML files.

Configuration
-------------

The XML converter supports the following configuration keys:

===================== ======== ======= ==========================================================================================
Key                   Required Type    Description
===================== ======== ======= ==========================================================================================
``type``              yes      String  Must be the string ``xml``.
``feature-path``      no       String  An XPath expression used to select multiple features from a single document.
``xpath-factory``     no       String  An alternate ``javax.xml.xpath.XPathFactory`` to use for parsing.
``xml-namespaces``    no       Object  XML namespace prefix mappings. See :ref:`xml_converter_namespaces` for details.
``xsd``               no       String  Name of an XSD file on the classpath used to validate input documents.
``options.line-mode`` no       String  The line format of the input being parsed - either ``multi`` (default) or ``single``.
===================== ======== ======= ==========================================================================================

``feature-path``
^^^^^^^^^^^^^^^^

Since a single XML document may contain multiple features, the XML converter supports an XPath expression pointing to each
feature element.
...
faster than the default Java implementation. If desired, a different parser may be specified by setting ``xpath-factory``
...
Files can be parsed in either ``single``-line mode or in ``multi``-line mode. In single-line mode, each line of an input file
should be a valid XML document; in multi-line mode, the entire input file should be a single valid XML document. In order to
support XPath expressions, each XML document is fully parsed into memory. For large documents, this may take considerable time
and memory. Thus, it is usually better to use single-line mode when possible. If nothing is specified, multi-line mode is used.

Field Configuration
-------------------

The ``fields`` element in an XML converter supports additional keys:

============= ==========================================================================================================
Key           Description
============= ==========================================================================================================
``path``      An XPath expression used to extract the field value.
============= ==========================================================================================================

``path``
^^^^^^^^

``path`` should be a XPath expression, which may be relative to the ``feature-path`` (if defined, above) or absolute to the
document root. The ``path`` expression will be evaluated to a string, and be available in the ``transform`` as ``$0``.
...
In addition to the standard :ref:`converter_functions`, the XML converter provides the following XML-specific functions:
...
omitted if namespaces are not used. Generally, the ``parseMap`` function (see :ref:`converter_list_map_fns`) will be required
to create an appropriate namespace map. See :ref:`xml_converter_namespaces`, below.
...
``xpath`` function (above). See :ref:`composite_predicates` for more details on predicates.
