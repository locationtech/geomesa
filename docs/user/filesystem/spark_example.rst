...
.. code-block:: bash
    ...
    $ ls dist/spark
    geomesa-fs-spark_{{scala_binary_version}}-{{release}}.jar  geomesa-fs-spark_{{scala_binary_version}}-{{release}}-runtime.jar
...
.. code-block:: bash
    ...
    . /etc/hadoop/conf/yarn-env.sh
    export HADOOP_CONF_DIR=/etc/hadoop/conf
    spark-shell --jars $GEOMESA_FS_HOME/dist/spark/geomesa-fs-spark_{{scala_binary_version}}-{{release}}-runtime.jar --driver-memory 3g
