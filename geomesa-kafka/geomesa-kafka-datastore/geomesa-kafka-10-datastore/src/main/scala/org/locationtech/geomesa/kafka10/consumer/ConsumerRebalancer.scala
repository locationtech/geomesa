...
class ConsumerRebalancer[K, V](consumer: KafkaConsumer[K, V], config: ConsumerConfig)
  ...
  // used to register the consumer in zk
  private val topicPartitions = mutable.Map.empty[String, Set[Int]]
  // partitions that the consumer owns
  ...
  def syncedRebalance(): Unit = rebalanceLock.synchronized {
    rebalance(1)
  }
...
object ConsumerRebalancer {
  ...
  /**
    ...
    */
  def getCluster(zkUtils: ZkUtils10): Seq[Broker] =
