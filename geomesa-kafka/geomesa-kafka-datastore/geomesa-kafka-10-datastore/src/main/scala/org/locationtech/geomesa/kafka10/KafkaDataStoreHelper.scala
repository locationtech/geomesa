...
object KafkaDataStoreHelper {
  ...
  private[kafka10] def insertTopic(sft: SimpleFeatureType, topic: String): Unit =
    sft.getUserData.put(TopicKey, topic)

  private[kafka10] def extractTopic(sft: SimpleFeatureType): Option[String] = sft.userData[String](TopicKey)

  /** Modifies the passed SimpleFeatureType, adding the encoded replay configString to the user data */
  private[kafka10] def insertReplayConfig(sft: SimpleFeatureType, configString: String): Unit =
    sft.getUserData.put(ReplayConfigKey, configString)

  private[kafka10] def extractReplayConfig(sft: SimpleFeatureType): Option[ReplayConfig] =
  ...
  private[kafka10] def buildTopicName(zkPath: String, sft: SimpleFeatureType): String = {
