...
import org.locationtech.geomesa.kafka.index.state.FeatureStateFactory.{ExpiredState, ExtentsExpiryState, FeatureExpiration, FeatureState}
...
/**
  * Non-point feature index with event time expiration but message time ordering
  *
  ...
  */
class ExtentsEventTimeExpiryFeatureStateFactory(index: SpatialIndex[SimpleFeature],
                                                geom: Int,
