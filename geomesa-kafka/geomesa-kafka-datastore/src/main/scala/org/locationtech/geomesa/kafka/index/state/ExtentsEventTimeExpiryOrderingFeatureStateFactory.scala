...
/**
  * Non-point feature index with event time ordering and timed expiration
  *
  ...
  */
class ExtentsEventTimeExpiryOrderingFeatureStateFactory(index: SpatialIndex[SimpleFeature],
                                                        geom: Int,
  ...
  override def createState(feature: SimpleFeature): FeatureState = {
    ...
    if (expiry < 1L) {
      ...
      new ExtentsExpiryState(feature, index, geom, time, expiration, executor, expiry)
    }
  }
