...
/**
  * Point feature state with event time ordering
  *
  * @param index spatial index
  * @param geom geometry attribute
  * @param eventTime event time expression
  */
class PointsEventTimeFeatureStateFactory(index: SpatialIndex[SimpleFeature], geom: Int, eventTime: Expression)
    extends FeatureStateFactory with LazyLogging {

  override def createState(feature: SimpleFeature): FeatureState =
    new PointState(feature, index, geom, FeatureStateFactory.time(eventTime, feature))
