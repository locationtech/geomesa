...
class OffsetManager(val config: ConsumerConfig)
  ...
  lazy private val zkUtils = KafkaUtils08.createZkUtils(config)
  ...
  private def commitOffsets(offsets: Map[TopicAndPartition, OffsetAndMetadata], tries: Int): Unit = {
    ...
    try {
      ...
      val response = OffsetCommitResponse.readFrom(KafkaUtils08.channelToPayload(channel.channel()))
...
object OffsetManager extends LazyLogging {
  ...
  def getGroupOffsets(channel: BlockingChannel,
    ...
    val response = OffsetFetchResponse.readFrom(KafkaUtils08.channelToPayload(channel))
