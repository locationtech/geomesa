...
object KafkaDataStoreHelper {
  ...
  private[kafka09] def insertTopic(sft: SimpleFeatureType, topic: String): Unit =
    sft.getUserData.put(TopicKey,topic)

  private[kafka09] def extractTopic(sft: SimpleFeatureType) : Option[String] = sft.userData[String](TopicKey)

  /** Modifies the passed SimpleFeatureType, adding the encoded replay configString to the user data */
  private[kafka09] def insertReplayConfig(sft: SimpleFeatureType, configString : String): Unit =
    sft.getUserData.put(ReplayConfigKey, configString)

  private[kafka09] def extractReplayConfig(sft: SimpleFeatureType) : Option[ReplayConfig] =
  ...
  private[kafka09] def buildTopicName(zkPath: String, sft: SimpleFeatureType): String = {
