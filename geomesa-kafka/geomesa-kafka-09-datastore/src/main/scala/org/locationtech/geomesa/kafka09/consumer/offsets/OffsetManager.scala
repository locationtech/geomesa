...
class OffsetManager(val config: ConsumerConfig)
  ...
  @tailrec
  private[kafka09] final def binaryOffsetSearch(consumer: WrappedConsumer,
                                                predicate: MessagePredicate,
  ...
  private[kafka09] def trim(messageSet: ByteBufferMessageSet, end: Long): Array[MessageAndOffset] =
 ...
 */
case class WrappedChannel(zkUtils: ZkUtils09, config: ConsumerConfig) {
