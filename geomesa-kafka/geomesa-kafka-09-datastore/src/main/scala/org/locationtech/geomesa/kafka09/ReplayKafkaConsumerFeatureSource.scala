...
import org.locationtech.geomesa.kafka09.consumer.KafkaStreamLike.KafkaStreamLikeIterator
import org.locationtech.geomesa.kafka09.consumer.offsets.FindOffset
...
class ReplayKafkaConsumerFeatureSource(entry: ContentEntry,
  ...
  private[kafka09] val messages: Array[GeoMessage] = readMessages()
  ...
  private[kafka09] def snapshot(time: Option[Long]): Option[ReplaySnapshotFeatureCache] = {
  ...
  /** @return the index of the most recent [[GeoMessage]] at or before the given ``time``
    */
  private[kafka09] def indexAtTime(time: Long): Option[Int] = {
...
object ReplayTimeHelper {
  ...
  private[kafka09] val ff = CommonFactoryFinder.getFilterFactory2
...
/** Represents the state at a specific point in time.
  ...
  *               [[Delete]] messages
  */
private[kafka09] case class ReplaySnapshotFeatureCache(override val sft: SimpleFeatureType,
                                                     replayTime: Long,
