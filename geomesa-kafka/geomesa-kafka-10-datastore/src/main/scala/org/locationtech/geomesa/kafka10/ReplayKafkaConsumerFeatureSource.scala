...
import org.locationtech.geomesa.kafka10.consumer.KafkaStreamLike._
...
class ReplayKafkaConsumerFeatureSource(entry: ContentEntry,
  ...
  private[kafka10] val messages: Array[GeoMessage] = {
  ...
  private[kafka10] def snapshot(time: Option[Long]): Option[ReplaySnapshotFeatureCache] = {
  ...
  /** @return the index of the most recent [[GeoMessage]] at or before the given ``time``
    */
  private[kafka10] def indexAtTime(time: Long): Option[Int] = {
...
object ReplayTimeHelper {
  ...
  private[kafka10] val ff = CommonFactoryFinder.getFilterFactory2
...
/** Represents the state at a specific point in time.
  ...
  *               [[Delete]] messages
  */
private[kafka10] case class ReplaySnapshotFeatureCache(override val sft: SimpleFeatureType,
                                                       replayTime: Long,
