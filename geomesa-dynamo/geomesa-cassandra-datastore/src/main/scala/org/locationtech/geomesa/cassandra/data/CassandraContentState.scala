...
class CassandraContentState(ent: ContentEntry, val session: Session, val tableMetadata: TableMetadata)
  ...
  val deserializers = sft.getAttributeDescriptors.map { ad => FieldSerializer(ad) }.zipWithIndex
