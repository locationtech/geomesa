...
object Transformers extends EnhancedTokenParsers with LazyLogging {
  ...
  def reloadFunctions(): Unit = TransformerParser.functionMap.putAll(loadFunctions())

  private def loadFunctions(): java.util.Map[String, TransformerFn] = {
    val map = new java.util.HashMap[String, TransformerFn]
    ServiceLoader.load(classOf[TransformerFunctionFactory]).foreach { factory =>
      factory.functions.foreach(f => f.names.foreach(map.put(_, f)))
    }
    map
  }
  ...
  object TransformerParser {
    ...
    private [Transformers] lazy val functionMap = new ConcurrentHashMap[String, TransformerFn](loadFunctions())
    ...
    def fn          = (fnName <~ OPEN_PAREN) ~ (repsep(argument, ",") <~ CLOSE_PAREN) ^^ {
      case LitString(name) ~ e => {
        val function = Option(functionMap.get(name)).getOrElse {
          throw new IllegalArgumentException(s"Could not find transformer function '$name'")
        }
        FunctionExpr(function.getInstance, e.toArray)
      }
    }
