...
import org.locationtech.geomesa.convert.{DefaultCounter, EvaluationContextImpl, SimpleFeatureConverters, Transformers}
...
class ScriptingFunctionFactoryTest extends Specification {
  ...
  System.setProperty(ScriptingFunctionFactory.ConvertScriptsPath.property, path)
  Transformers.reloadFunctions()
  ...
  "ScriptingFunctionFactory " should {
    ...
    "execute functions" >> {
      val ec = new EvaluationContextImpl(IndexedSeq.empty[String], Array.empty[Any], new DefaultCounter, Map.empty)
      val fn = sff.functions.find(_.names.contains("js:hello")).head
      val res = fn.eval(Array("geomesa"))(ec)
