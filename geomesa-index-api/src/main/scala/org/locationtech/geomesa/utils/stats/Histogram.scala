...
object Histogram {
  ...
  def buffer[T](value: T): (T, T) = {
    ...
    val buf = value match {
      ...
      case v: Geometry =>
        val env = safeCentroid(v).buffer(10.0).getEnvelopeInternal
        val min = geoFactory.createPoint(new Coordinate(env.getMinX, env.getMinY))
        val max = geoFactory.createPoint(new Coordinate(env.getMaxX, env.getMaxY))
        (min, max)
    }
