...
object Stat {
  ...
  private val DateFormat = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'").withZone(ZoneOffset.UTC)

  private val WktReaders = new ThreadLocal[WKTReader] {
    override def initialValue = new WKTReader()
  }
  private val WktWriters = new ThreadLocal[WKTWriter] {
  ...
  def stringifier[T](clas: Class[T], json: Boolean = false): Any => String = {
    val toString: Any => String = if (classOf[Geometry].isAssignableFrom(clas)) {
      v => WktWriters.get.write(v.asInstanceOf[Geometry])
    } else if (classOf[Date].isAssignableFrom(clas)) {
      v => DateFormat.format(v.asInstanceOf[Date].toInstant)
    } else {
      v => v.toString
    }
  ...
  def destringifier[T](clas: Class[T]): String => T = {
    if (clas == classOf[String]) {
      ...
      s => if (s == "null") { null.asInstanceOf[T] } else { WktReaders.get.read(s).asInstanceOf[T] }
    } else if (classOf[Date].isAssignableFrom(clas)) {
      s => if (s == "null") { null.asInstanceOf[T] } else {
        Date.from(LocalDateTime.parse(s, DateFormat).toInstant(ZoneOffset.UTC)).asInstanceOf[T]
      }
    } else {
