  ...
  override def getSchema(typeName: String): SimpleFeatureType = {
    ...
    schemas.reduceLeft[SimpleFeatureType] { case (left, right) =>
      ...
      while (leftDescriptors.hasNext) {
        ...
        builder.add(attribute.buildDescriptor(leftDescriptor.getLocalName))
      }
      namespace.foreach(builder.setNamespaceURI)
      builder.setName(typeName)
      val sft = builder.buildFeatureType()
      sft.getUserData.putAll(left.getUserData)
      sft.getUserData.putAll(right.getUserData)
      sft
    }
  }
