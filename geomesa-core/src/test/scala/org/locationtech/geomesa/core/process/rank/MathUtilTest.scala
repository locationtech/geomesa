...
class MathUtilTest extends Specification {
  ...
  "MathUtil" should {
    ...
    "calculate NEWS headings correctly" in {
      ...
      MathUtil.headingGivenRadians(refR, d(0.0, 1.0)) must beCloseTo(0.0, epsilon) // head North
      MathUtil.headingGivenRadians(refR, d(1.0, 0.0)) must beCloseTo(89.678601, epsilon) // head East(ish)
      MathUtil.headingGivenRadians(refR, d(-1.0, 0.0)) must beCloseTo(-89.678601, epsilon) // head West(ish)
      MathUtil.headingGivenRadians(refR, d(0.0, -1.0)) must beCloseTo(180.0, epsilon) // head South
    }

    "calculate straightforward headingDifference correctly" in {
      MathUtil.headingDifference(-10.0, 10.0) must beCloseTo(20.0, epsilon)
    }
