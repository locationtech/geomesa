  ...
  "AttributeIndexIterator" should {
    ...
    "return correct results" >> {
      ...
      "for string equals" >> {
        ...
        results.map(_.getAttribute("name").asInstanceOf[String]) must contain(equalTo("b")).foreach
      ...
      "for string less than" >> {
        ...
        results must haveSize(4)
        results.map(_.getAttributeCount) must contain(3).foreach
        results.map(_.getAttribute("name").asInstanceOf[String]) must contain(equalTo("a")).foreach
        results.map(_.getAttribute("geom").toString) must contain("POINT (45 45)", "POINT (46 46)", "POINT (47 47)", "POINT (48 48)")
        results.map(_.getAttribute("dtg").asInstanceOf[Date]) must contain(dateToIndex).foreach
      }
      ...
      "for string greater than" >> {
        ...
        results.map(_.getAttributeCount) must contain(3).foreach
        results.map(_.getAttribute("name").asInstanceOf[String]) must contain(equalTo("c")).exactly(4)
        results.map(_.getAttribute("name").asInstanceOf[String]) must contain(equalTo("d")).exactly(4)
        results.map(_.getAttribute("geom").toString) must contain("POINT (45 45)", "POINT (46 46)", "POINT (47 47)", "POINT (48 48)")
        results.map(_.getAttribute("dtg").asInstanceOf[Date]) must contain(dateToIndex).foreach
      }
      ...
      "for string greater than or equals" >> {
        ...
        results.map(_.getAttributeCount) must contain(3).foreach
        results.map(_.getAttribute("name").asInstanceOf[String]) must contain(equalTo("b")).exactly(4)
        results.map(_.getAttribute("name").asInstanceOf[String]) must contain(equalTo("c")).exactly(4)
        results.map(_.getAttribute("name").asInstanceOf[String]) must contain(equalTo("d")).exactly(4)
        results.map(_.getAttribute("geom").toString) must contain("POINT (45 45)", "POINT (46 46)", "POINT (47 47)", "POINT (48 48)")
        results.map(_.getAttribute("dtg").asInstanceOf[Date]) must contain(dateToIndex).foreach
      ...
      "for int less than" >> {
        ...
        results.map(_.getAttribute("geom").toString) must contain(equalTo("POINT (45 45)")).foreach
        results.map(_.getAttribute("dtg").asInstanceOf[Date]) must contain(dateToIndex).foreach
      }
      ...
      "for int greater than or equals" >> {
        ...
        results.map(_.getAttribute("geom").toString) must contain(equalTo("POINT (47 47)")).exactly(5)
        results.map(_.getAttribute("geom").toString) must contain(equalTo("POINT (48 48)")).exactly(5)
        results.map(_.getAttribute("dtg").asInstanceOf[Date]) must contain(dateToIndex).foreach
      }
      ...
      "not including geom or dtg" >> {
        ...
        results must haveSize(4)
        results.map(_.getAttributeCount) must contain(2).foreach // geom gets added back in
        results.map(_.getAttribute("name").toString) must contain(equalTo("b")).foreach
        results.map(_.getAttribute("geom").toString) must contain("POINT (45 45)", "POINT (46 46)", "POINT (47 47)", "POINT (48 48)")
      }

      "with additional filter applied" >> {
        val filter = filterFactory.and(ECQL.toFilter("name = 'b'"), ECQL.toFilter("BBOX(geom, 44.5, 44.5, 45.5, 45.5)"))
