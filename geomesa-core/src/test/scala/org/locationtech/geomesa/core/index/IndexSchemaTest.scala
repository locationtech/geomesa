...
class IndexSchemaTest extends Specification {
  ...
  "SpatioTemporalIndexSchemaTest" should {
    "parse a valid string" in {
      val schema = IndexSchema.buildKeyEncoder("%~#s%foo#cstr%99#r::%~#s%0,4#gh::%~#s%4,3#gh%15#id",
        dummyEncoder, dummyIndexValueEncoder)
    ...
    "allow index/data flags" in {
      val schema = IndexSchema.buildKeyEncoder("%~#s%#i%foo#cstr%99#r::%~#s%0,4#gh::%~#s%4,3#gh%15#id",
        dummyEncoder, dummyIndexValueEncoder)
      schema should not be null
    }
  }
  ...
  val index = IndexSchema.buildKeyEncoder(schemaEncoding, dummyEncoder, dummyIndexValueEncoder)
  ...
  "index-entry encoded and decoder" should {
    "encode and decode round-trip properly using a custom date-time field name" in {
      ...
      val encoder = IndexSchema.buildKeyEncoder(schema, customEncoder, dummyIndexValueEncoder)
