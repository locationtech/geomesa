...
trait AttributeIdxStrategy extends Strategy with Logging {
  ...
  def partitionFilter(filter: Filter, sft: SimpleFeatureType): (Query, Filter) = {
    ...
    def extractIndexProperty(and: And): (Seq[Filter], Filter) = {

      def extract(unprocessedFilters: Seq[Filter], processedFilters: Seq[Filter], attrIndexFilter: Option[Filter]): (Seq[Filter], Option[Filter]) = {
        if (unprocessedFilters.isEmpty) {
          (processedFilters, attrIndexFilter)
        } else {
          val head = unprocessedFilters.head
          if (attrIndexFilter.isEmpty && AttributeIndexStrategy.getAttributeIndexStrategy(head, sft).isDefined) {
            extract(unprocessedFilters.tail, processedFilters, Some(head))
          } else {
            extract(unprocessedFilters.tail, processedFilters ++ Seq(head), attrIndexFilter)
          }
        }
      }

      val (cqlFilter, indexFilterOption) = extract(and.getChildren, Seq(), None)
      (cqlFilter, indexFilterOption.get)
    }

    val (cqlFilter, indexFilter) = filter match {
      case and: And =>
        extractIndexProperty(and)
      case f: Filter =>
        (Seq(), f)
    }

    (new Query(sft.getTypeName, filterListAsAnd(cqlFilter).getOrElse(Filter.INCLUDE)), indexFilter)
  }
}
...
class AttributeIdxEqualsStrategy extends AttributeIdxStrategy {
  ...
  override def execute(acc: AccumuloConnectorCreator,
                       ...
                       query: Query,
                       output: ExplainerOutputType): SelfClosingIterator[Entry[Key, Value]] = {
    val (cqlQuery, indexFilter) = partitionFilter(query.getFilter, featureType)
    val range =
      indexFilter match {
        ...
        case _ =>
          val msg = s"Unhandled filter type in equals strategy: ${indexFilter.getClass.getName}"
          throw new RuntimeException(msg)
      }

    attrIdxQuery(acc, cqlQuery, iqp, featureType, range, output)
  }
}
  ...
  override def execute(acc: AccumuloConnectorCreator,
    ...
    val range =
      indexFilter match {
        ...
        case _ =>
          val msg = s"Unhandled filter type in range strategy: ${indexFilter.getClass.getName}"
          throw new RuntimeException(msg)
      }
...
class AttributeIdxLikeStrategy extends AttributeIdxStrategy {
  ...
  override def execute(acc: AccumuloConnectorCreator,
                       ...
                       output: ExplainerOutputType): SelfClosingIterator[Entry[Key, Value]] = {
    val (cqlQuery, indexFilter) = partitionFilter(query.getFilter, featureType)
    val filter = indexFilter.asInstanceOf[PropertyIsLike]
