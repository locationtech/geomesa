...
class CassandraDataStoreTest extends Specification with BeforeAfterAll {
  ...
  def beforeAll(): Unit = {
    ...
    dsList = List(dsFromContainer(cassandraContainer), dsFromContainer(scyllaContainer))
  }
  ...
  def dsFromContainer(container: GenericContainer[?]): CassandraDataStore = {
    ...
    DataStoreFinder.getDataStore(params.asJava).asInstanceOf[CassandraDataStore]
  }
  ...
  "CassandraDataStore" should {
    ...
    "work with points" in {
      ...
      def test(ds: CassandraDataStore) = {
