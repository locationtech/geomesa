...
class CassandraSpatialRDDProvider extends SpatialRDDProvider with LazyLogging {
  ...
  override def canProcess(params: java.util.Map[String, _ <: java.io.Serializable]): Boolean = {
    val ret = CassandraDataStoreFactory.canProcess(params)
    println(s"CanProcess? $ret with params: $params")
    ret
  }
...
//          case stmt: RegularStatement =>
//            println(s"JNH: Setting statement: \n${stmt.getQueryString}")
//            CqlConfigHelper.setInputCql(
//            config,
//            stmt.getQueryString
//          )
...
//        }
    ...
    try {
      ...
      lazy val qps = {
        CassandraJobUtils1.getMultiScanPlans(ds, origQuery)
      }
