...
import org.apache.hadoop.hbase.client.{HTable, RegionLocator, Scan}
...
class HBaseDensityFilterTest extends Specification with LazyLogging {
  ...
  val className = "org.locationtech.geomesa.hbase.data.HBaseDensityFilterTest$TestTableSplit"

  val TEST_FAMILY = s"an_id:java.lang.Integer,attr:java.lang.Double,dtg:Date,geom:Point:srid=4326;table.splitter.class=${className}"
...
//    CoprocessorThreadsParam.getName -> "2"
  ...
  step {
    val foo = new TestTableSplit
    println(s"Class name: ${foo.getClass}")
...
//    }
    ...
    "cover a diagonal" in {
      ...
      println(s"Region Servers: ${ds.connection.getAdmin.getRegionServers.mkString(", ")}")

//      val tables = ds.getAllTableNames(typeName)
//      tables.foreach { t => println(s"Tables: $t") }
//
//      val tableName = "HBaseDensityFilterTest_HBaseDensityFilterTest_z3_geom_dtg_v6"
//      println(s" Dumping table ${tableName}")
//      val table = ds.connection.getTable(TableName.valueOf(tableName))
//      val scanner = table.getScanner(new Scan())
//      var i = 0
//      scanner.iterator().foreach { result =>
//        val row = result.getRow
//        println(s"Array(${row.map(_.toInt).mkString(",")}),")
//        i += 1
//      }
