...
object CqlTransformFilter extends StrictLogging {
  ...
  private def serialize(delegate: DelegateFilter): Array[Byte] = {
    ...
    delegate.transform match {
      case None =>
        val array = Array.ofDim[Byte](sftBytes.length + cqlBytes.length + indexBytes.length + 16)
  ...
  private def deserializeIndex(
      ...
      spec: String,
      bytes: Array[Byte],
      start: Int): GeoMesaFeatureIndex[_, _, _] = {
    if (bytes.length <= start) {
      ...
      offset += 4
      HBaseFeatureIndex.index(new String(bytes, offset, identifierLength, StandardCharsets.UTF_8))
    }
  }
  ...
  private sealed trait DelegateFilter {
    ...
    def index: GeoMesaFeatureIndex[_, _, _]
  ...
  private class FilterDelegate(
      ...
      val index: GeoMesaFeatureIndex[_, _, _],
      feature: KryoBufferSimpleFeature,
      filt: Filter
    ) extends DelegateFilter {

    private val getIdFromRow: (Array[Byte], Int, Int, SimpleFeature) => String =
      if (index == null) { (_, _, _, _) => "" } else { index.getIdFromRow(sft) }
    ...
    override def filterKeyValue(v: Cell): ReturnCode = {
      try {
        feature.setId(getIdFromRow(v.getRowArray, v.getRowOffset, v.getRowLength, null))
