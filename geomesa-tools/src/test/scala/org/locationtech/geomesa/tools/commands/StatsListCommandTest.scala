...
class StatsListCommandTest extends Specification {

  "StatsListCommand" should {
    "put points on the map appropriately" >> {
      skipped("integration")

      val sft = SimpleFeatureTypes.createType("StatsListCommandTest", "geom:Geometry:srid=4326")
      val stat = Stat(sft, Stat.RangeHistogram[Geometry]("geom", 1024, MinMaxGeometry.max, MinMaxGeometry.min))
      ...
      println(StatsListCommand.geomHistToString("geom", histogram))
