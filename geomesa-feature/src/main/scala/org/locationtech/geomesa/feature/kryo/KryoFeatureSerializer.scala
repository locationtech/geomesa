...
object KryoFeatureSerializer {
  ...
  def apply(sft: SimpleFeatureType, options: Set[EncodingOption] = Set.empty): KryoFeatureSerializer =
    apply(new SimpleFeatureSerializer(sft, options))

  def apply(sft: SimpleFeatureType, decodeAs: SimpleFeatureType, options: Set[EncodingOption]): KryoFeatureSerializer = {
    if (sft.eq(decodeAs))
      apply(sft, options)
